...
bool Circle::CrossesSegment(const Segment &segment) const {
  ...
  int64_t numerator_sqr = std::pow(
      coefficient_x * c.GetX() + coefficient_y * c.GetY() + free_coefficient,
      2);
  int64_t denominator_sqr =
      std::pow(coefficient_x, 2) + std::pow(coefficient_y, 2);
  int64_t distance_a_c_sqr =
      std::pow(a.GetX() - c.GetX(), 2) + std::pow(a.GetY() - c.GetY(), 2);
  int64_t distance_b_c_sqr =
      std::pow(b.GetX() - c.GetX(), 2) + std::pow(b.GetY() - c.GetY(), 2);
  if (ScalarMultiplication(vector_a_b, vector_a_c) > 0) {
    if (ScalarMultiplication(vector_a_b, vector_b_c) < 0) {
      ...
      if (numerator_sqr < radius_effective * denominator_sqr) {
        ...
        return ((vector_1.LenSqr() - radius_ * radius_ >= 0) ||
                (vector_2.LenSqr() - radius_ * radius_ >= 0));
      }
      return false;
    }
