...
Segment::Segment(const Point &point_1, const Point &point_2)
    : p_one_(point_1), p_two_(point_2) {}
...
Point Segment::GetOne() const { return p_one_; }

Point Segment::GetTwo() const { return p_two_; }
...
bool OnTheBorder(const Point &a_1, const Point &b_1, const Point &a_2,
                 const Point &b_2) {
...
bool Intersection(const Point &a_1, const Point &b_1, const Point &a_2,
                  const Point &b_2) {
  ...
  if (std::abs(mult_2_3) + std::abs(mult_2_5) + std::abs(mult_1_3) +
          std::abs(mult_1_4) ==
      0) {
...
std::string Segment::ToString() const {
  std::string string =
      "Segment(" + p_one_.ToString() + ", " + p_two_.ToString() + ")";
  return string;
}
} // namespace geometry
