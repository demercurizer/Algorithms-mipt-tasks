...
Point Ray::GetOne() const { return p_one_; }

Point Ray::GetTwo() const { return p_two_; }
...
bool Ray::CrossesSegment(const Segment &s) const {
  ...
  bool result = (VectorMultiplication(vector_a_o, vector_a_b) *
                     VectorMultiplication(v, vector_a_b) <
                 0);
  return result;
}
