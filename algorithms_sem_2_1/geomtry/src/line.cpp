...
Line::Line(const int64_t &a_input, const int64_t &b_input,
           const int64_t &c_input)
    : a_(a_input), b_(b_input), c_(c_input), p_one_(Point()), p_two_(Point()) {}
...
bool Line::CrossesSegment(const Segment &segment) const {
  bool result =
      ((a_ * segment.GetOne().GetX() + b_ * segment.GetOne().GetY() + c_) *
           (a_ * segment.GetTwo().GetX() + b_ * segment.GetTwo().GetY() + c_) <=
       0);
  return result;
}
